...
export class PanelBuilder {
    ...
    render(config) {
        ...
        // Omarchy-style tray drawer (foreign SNI icons behind chevron)
        try { this._trayDrawer?.destroy?.(); } catch (e) {}
        this._trayDrawer = null;
        // Default ON (Omarchy-style). Explicit false disables.
        ...
        if (useDrawer) {
            try {
                ...
                log('material-panel: tray drawer enabled');
            } catch (e) {
                logError(e, 'material-panel: tray drawer');
                this._bridge.setTrayHost(null, false);
            }
        } else {
            this._bridge.setTrayHost(null, false);
            log('material-panel: tray drawer disabled');
        }
