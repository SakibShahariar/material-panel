...
import {buildBluetooth, isBluetoothPopupReady} from '../modules/bluetooth.js';
...
export class PanelBuilder {
    ...
    render(config) {
        ...
        this._ensureBluetoothPopupHost(scale);
    ...
    _ensureBluetoothPopupHost(scale) {
        try {
            if (isBluetoothPopupReady())
                return;
            const btn = buildBluetooth(this._extensionPath, scale);
            if (!btn)
                return;
            btn.visible = false;
            btn.opacity = 0;
            try {
                this._panelActor.add_child(btn);
            } catch (e) {
                try { this._zones?.right?.add?.(btn); } catch (e2) {}
            }
            log('material-panel: bluetooth popup host created (hidden)');
        } catch (e) {
            logError(e, 'material-panel: ensureBluetoothPopupHost');
        }
    }
    ...
    _attachTrayIconStrip() {
        ...
        try {
            ...
            try {
                box.style = 'spacing: 4px; padding: 0 6px; min-height: 24px;';
            } catch (e) {}
