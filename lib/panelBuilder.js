    ...
    _verifyModules(config, gen) {
        try {
            ...
            for (const z of ['left', 'center', 'right']) {
                for (const id of preset?.zones?.[z] ?? []) {
                    if (!hidden.has(id) && !String(id).startsWith('extension:'))
                        expected.push(id);
                }
            }
            // end4 uses fixed lists
            if ((config.layoutStyle ?? 'default') === 'end4') {
                // soft check only
            }
            ...
            log(`material-panel: module verify expected~${expected.length} zoneChildren=${childCount}`);
            if (expected.length > 0 && childCount < Math.min(3, expected.length)) {
                log('material-panel: module verify LOW — scheduling one recovery render');
