...
export class PanelBuilder {
    ...
    render(config) {
        ...
        this._attachSniStrip();
        this._bridge.reattachAll();
        try {
            log(`material-panel: right zone children=${this._zones?.right?.actor?.get_n_children?.() ?? 'n/a'} sniStrip=${!!this._sniStrip}`);
        } catch (e) {}
    ...
    _attachSniStrip() {
        ...
        try {
            const box = new St.BoxLayout({
                style_class: 'material-panel-sni-strip',
                y_align: Clutter.ActorAlign.CENTER,
            ...
            log(`material-panel: SNI strip attached (right children=${z?.actor?.get_n_children?.() ?? '?'})`);
        } catch (e) {
            logError(e, 'material-panel: SNI strip');
            this._sniStrip = null;
        }
    }
    ...
    _teardown() {
        this._sniStrip = null;
        try { this._bridge?.setSniStrip?.(null); } catch (e) {}
        try { this._trayDrawer?.destroy?.(); } catch (e) {}
        this._trayDrawer = null;
