...
import {iconPath, iconPathPrimary, iconPathOnAccent, resolveIconPath} from './iconTheme.js';
...
export function tintSymbolic(icon, color) {
    if (!icon)
        return;
    try {
        // Chevrons/arrows are sized+colored by CSS per-state; skip auto-tint
        if (String(icon.style_class || '').includes('material-panel-drop-chevron'))
            return;
    } catch (e) {}
    try {
...
export function giconForKey(key, onAccent, resting = 'primary') {
    try {
        // Prefer resolved path (current gen, then previous) so a failed TextureCache
        // decode of a unique -g{N} URL can still show an older good file.
        let variant = 'plain';
        if (onAccent)
            variant = 'on-accent';
        ...
        let p = resolveIconPath(key, variant);
        if (!Gio.File.new_for_path(p).query_exists(null))
            p = resolveIconPath(key, 'plain');
        if (Gio.File.new_for_path(p).query_exists(null))
...
export function wireChipPress(actor, {
    getIcons = null,
    // false = clear pressed on pointer release (correct button behavior)
    // true = keep pressed until leave (legacy); prefer bindMenuPress for open menus
    ...
    const apply = () => {
        ...
        for (const entry of list) {
            ...
            if (!entry?.icon)
                continue;
            try {
                if (entry.icon.actor_destroyed || entry.icon._materialPanelDisposed)
                    continue;
            } catch (e) {
                continue;
            }
    ...
    // Some popups never deliver button-release-event to the pressed actor
    // (GNOME emits 'clicked' instead), so '.pressed' would stay latched until
    // leave-event. Clear it on the click itself for non-sticky buttons.
    try {
        if (!stickyUntilLeave)
...
/** Keep chip pressed style while its popup menu is open. */
export function bindMenuPress(menu, pressState) {
    if (!menu || !pressState)
        return;
    try {
        menu.connect('open-state-changed', (_m, open) => {
            pressState.pressed = !!open;
            try { pressState.applyIcons?.(); } catch (e) {}
        });
    } catch (e) {}
}

export function wireFileIconPress(actor, getIcons) {
    return wireChipPress(actor, {getIcons, stickyUntilLeave: false, restingIcon: 'primary'});
}

export function wirePressedClass(actor) {
    return wireChipPress(actor, {stickyUntilLeave: false});
}
