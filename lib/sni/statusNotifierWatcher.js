...
export class StatusNotifierWatcher {
    constructor(extension, watchDog) {
        ...
        // Pulse HostRegistered so Qt apps that started early re-check the watcher
        for (const sec of [2, 4, 8]) {
            GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, sec, () => {
                try {
                    this._dbusImpl.emit_signal('StatusNotifierHostRegistered', null);
                    log(`material-panel: SNI HostRegistered pulse ${sec}s`);
                } catch (e) {}
        ...
        // Second pass — late autostart apps / strip became ready
        GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 5, () => {
            this._seekStatusNotifierItems(extension).catch(e => {
                if (!e.matches(Gio.IOErrorEnum, Gio.IOErrorEnum.CANCELLED))
                    logError(e, 'SNI second seek');
            });
            return GLib.SOURCE_REMOVE;
        });
    }
    ...
    async _seekStatusNotifierItems(extension) {
        ...
        const cancellable = this._cancellable;
        await new PromiseUtils.TimeoutSecondsPromise(1, GLib.PRIORITY_DEFAULT, cancellable);
        const busAnalyzer = GLib.build_filenamev([
            extension.path, 'lib', 'sni', 'tools', 'busAnalyzer.js',
        ]);
    ...
    /**
     * Fast in-process probe: ListNames → Get Id on classic SNI paths.
     * Catches qBittorrent unique-name-only items without subprocess.
     */
    _quickProbePaths() {
        const cancellable = this._cancellable;
        Gio.DBus.session.call(
            ...
            (_c, res) => {
                let names;
                try {
                    [names] = Gio.DBus.session.call_finish(res).deep_unpack();
                } catch (e) {
                    return;
                }
                const candidates = names.filter(n =>
                    n.startsWith(':') || /statusnotifieritem/i.test(n));
                log(`material-panel: SNI quickProbe candidates=${candidates.length}`);
                for (const name of candidates) {
                    for (const path of ['/StatusNotifierItem', '/org/freedesktop/StatusNotifierItem']) {
                        const id = Util.indicatorId(name, name, path);
                        if (this._items.has(id))
                            continue;
                        Gio.DBus.session.call(
                            ...
                            Gio.DBusCallFlags.NONE, 300, cancellable,
                            (_c2, res2) => {
                                try {
                                    const [v] = Gio.DBus.session.call_finish(res2).deep_unpack();
                                    const itemId = v?.deep_unpack?.() ?? v;
                                    log(`material-panel: SNI quickProbe hit ${name}${path} Id=${itemId}`);
                                    this._registerItem(name, name, path).catch(e =>
                                        logError(e, 'material-panel: quickProbe register'));
                                } catch (e) {
                                    // not SNI on this path
                                }
                            });
                    }
                }
