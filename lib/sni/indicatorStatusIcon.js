...
export function addIconToPanel(statusIcon) {
    ...
    const attachToStrip = () => {
        const host = globalThis._materialPanelSniHost;
        ...
        try {
            // Match material-panel chips
            try {
                statusIcon.add_style_class_name('material-panel-chip');
                statusIcon.add_style_class_name('material-panel-sni');
                statusIcon.add_style_class_name('material-panel-foreign');
                statusIcon.remove_style_class_name('panel-button');
            } catch (e) {}
            try {
                statusIcon.set_style(
                    'padding: 2px 8px; border-radius: 999px; min-height: 24px;');
            } catch (e) {}
            try {
                const box = statusIcon._box;
                if (box) {
                    box.add_style_class_name('material-panel-sni-inner');
                    box.set_style('spacing: 4px;');
                }
            } catch (e) {}
            // Prefer symbolic + theme primary (full-color pixmaps may ignore CSS)
            try {
                const icon = statusIcon._icon || statusIcon.icon;
                if (icon) {
                    icon.add_style_class_name('material-panel-sni-glyph');
                    try { icon.fallback_icon_name = icon.icon_name; } catch (e) {}
                }
            } catch (e) {}
            // Register for Tray prefs placement (left / right / center / off)
            try {
                globalThis._materialPanelRegisterSni?.(indicatorId, statusIcon);
            } catch (e) {}
            host.add(statusIcon);
            statusIcon.visible = true;
            statusIcon.opacity = 255;
            log(`material-panel: SNI full stack hosted ${indicatorId}`);
            return true;
        } catch (e) {
            logError(e, 'material-panel: SNI addIconToPanel');
            return false;
        }
    };
    ...
    // Strip not ready yet (login race) — retry; do NOT use hidden Main.panel
    log(`material-panel: SNI strip not ready for ${indicatorId} — retrying`);
    let n = 0;
    GLib.timeout_add(GLib.PRIORITY_DEFAULT, 250, () => {
        try {
            if (statusIcon.get_stage && !statusIcon.get_stage())
                return GLib.SOURCE_REMOVE; // destroyed
        } catch (e) {}
        if (attachToStrip())
            return GLib.SOURCE_REMOVE;
        if (++n > 60) { // ~15s
            log(`material-panel: SNI gave up hosting ${indicatorId}`);
            return GLib.SOURCE_REMOVE;
        }
