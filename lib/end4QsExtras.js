...
export function buildEnd4CalendarSection() {
    ...
    for (const d of ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su']) {
        ...
        style(lab, 'font-size: 10px; opacity: 0.55; width: 32px; text-align: center;');
        try { lab.width = 32; } catch (e) {}
        ...
        for (let i = 0; i < cells.length; i += 7) {
            ...
            for (let c = 0; c < 7; c++) {
                ...
                if (d === null) {
                    ...
                    try { spacer.width = 32; spacer.height = 28; } catch (e) {}
                    row.add_child(spacer);
                    continue;
                }
                const isToday = isThisMonth && d === todayDay;
                const wrap = new St.Bin({x_expand: false});
                try { wrap.width = 32; wrap.height = 28; } catch (e) {}
                const lab = new St.Label({
                    text: String(d),
                    x_align: Clutter.ActorAlign.CENTER,
                    y_align: Clutter.ActorAlign.CENTER,
