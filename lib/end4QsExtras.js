...
export function buildEnd4CalendarSection() {
    ...
    for (const d of ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su']) {
        ...
        style(lab, 'font-size: 10px; opacity: 0.55; width: 42px;');
        try { lab.width = 42; } catch (e) {}
        ...
        for (let i = 0; i < cells.length; i += 7) {
            ...
            for (let c = 0; c < 7; c++) {
                ...
                if (d === null) {
                    ...
                    try { spacer.width = 42; spacer.height = 28; } catch (e) {}
                ...
                try { cell.width = 42; cell.height = 28; } catch (e) {}
                if (isToday) {
                    style(cell,
                        'font-size: 11px; font-weight: 700; width: 42px; height: 28px;'
                        + ' background-color: #f5b8d0; color: #1a1a1a; border-radius: 999px;');
                } else {
                    style(cell, 'font-size: 11px; width: 42px; height: 28px; opacity: 0.9;');
                }
                row.add_child(cell);
            }
