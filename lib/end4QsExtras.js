    ...
    const rebuild = () => {
        ...
        for (let i = 0; i < cells.length; i += 7) {
            ...
            for (let c = 0; c < 7; c++) {
                ...
                try { cell.width = 36; cell.height = 28; } catch (e) {}
                if (isToday) {
                    style(cell,
                        'font-size: 11px; font-weight: 700; width: 36px; height: 28px;'
                        + ' background-color: #f5b8d0; color: #1a1a1a; border-radius: 999px;');
                } else {
                    style(cell, 'font-size: 11px; width: 36px; height: 28px; opacity: 0.9;');
                }
                row.add_child(cell);
            }
