...
`;
    ...
    // sourcePath: path to matugen's generated CSS (e.g.
    // ~/.config/matugen/matugen-colors.css). Missing/unreadable file is not
    // fatal — logs and leaves the bundled fixed palette in place.
    apply(sourcePath) {
        const vars = this._parseMatugenCss(sourcePath);
        if (!vars) {
            logError(new Error(
                `material-panel: matugen source not found at ${sourcePath}, keeping fixed palette`));
            return;
        }
