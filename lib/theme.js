...
`;
    ...
    _renderDimensions({scale = 1.0, gapTop = 8, gapBottom = 6, gapSide = 14, gap, layoutStyle = 'end4'} = {}) {
        ...
        const radius = Math.round(pillHeight * (end4 ? 0.95 : 0.85));
        ...
        const sideMargin = end4 ? Math.max(6, Math.round(8 * scale)) : 3;
        const zoneSpacing = end4 ? Math.max(4, Math.round(5 * scale)) : 8;
...
.material-panel-layout-end4 .material-panel-zone-center {
    /* stronger float separation */
    margin-left: ${px(sideMargin)};
    margin-right: ${px(sideMargin)};
}
.material-panel-clock {
...
`;
    ...
    apply(sourcePath, panelSize = {}, layoutStyle = 'end4') {
        let vars = {};
        if (sourcePath) {
