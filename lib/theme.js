...
`;
    ...
    // same class of bug fixed earlier when icon sizes grew past a fixed
    // bar height. Padding/radius/font/workspace-pill-size all scale
    // proportionally from pillHeight rather than being separate controls,
    // so the pill still looks coherent at any size rather than needing
    // five independently-tunable numbers to stay in proportion.
    _renderDimensions({iconScale = 1.0, pillHeight = 26, gap = 5} = {}) {
        const px = n => `${Math.round(n)}px`;
        // Exactly pillHeight + gap*2, no extra buffer - a prior version
        // added slack here "just in case", but that leftover space had
        // nothing anchoring how it got distributed above vs. below the
        // pill, and caused a visible top/bottom gap inconsistency. Icon
        // overflow (icon size vs. pill height) is a separate risk this
        // doesn't address - see the note on iconScale/pillHeight sliders.
        const barHeight = pillHeight + gap * 2;
        const padV = Math.max(2, Math.round(pillHeight * 0.15));
        ...
        const clockFont = (pillHeight * 0.48).toFixed(1);
        const workspaceSize = Math.round(pillHeight * 0.77);
        const workspaceFont = (pillHeight * 0.42).toFixed(1);
        const chipPadH = Math.max(6, Math.round(pillHeight * 0.4));
        const labelFont = (pillHeight * 0.48).toFixed(1);

        return `/* Generated bar dimensions — iconScale=${iconScale} pillHeight=${pillHeight} gap=${gap} */
.material-panel-bar {
    height: ${px(barHeight)};
