...
function recolor(svgText, hexColor) {
    // Iconify Material Symbols use fill="currentColor". GNOME/rsvg does not
    // resolve currentColor for FileIcon — icons render invisible if left as-is.
    // Old path only replaced #hex fills, then injected a second fill= on <path>,
    // leaving fill="currentColor" winning → empty chips after the end-4 icon swap.
    ...
    out = out.replace(/fill='#[0-9a-fA-F]{3,8}'/g, `fill='${hexColor}'`);
    // Paths / shapes still missing fill (Google 960-grid exports)
    out = out.replace(/<(path|circle|rect|polygon|polyline)(\s)(?![^>]*fill=)/g,
        `<$1 fill="${hexColor}"$2`);
    return out;
}
...
export function applyIcons(extensionPath, textColor, onAccentColor, primaryColor) {
    ...
    for (const key of ICON_KEYS) {
        ...
        try {
            writeRecolored(text, baseColor, iconPath(key));
        } catch (e) {
