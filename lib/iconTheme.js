...
function writeRecolored(srcContents, hexColor, destPath) {
    ...
    // GNOME's texture cache keys by file path, not content/mtime - without
    // this, a brand-new St.Icon actor pointing at this same path can still
    // get served a stale decoded texture from before this rewrite.
    try {
        St.TextureCache.get_default().uncache_file(destFile);
    } catch (e) {
        // Non-fatal - worst case the icon just doesn't refresh until next
        // full extension reload, same as before this fix existed.
    }
}
