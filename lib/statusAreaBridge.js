...
export class StatusAreaBridge {
    ...
    setForeignPlacements(map, _ignored = [], opts = {}) {
        ...
        // Old configs stamped everything "hidden" — treat as empty opt-in map
        if (!this._allHidden) {
        ...
        // Every role we know + every role in the map (opt-in may reference unseen keys)
        const roles = new Set([...this._known.keys(), ...this._roleZones.keys()]);
    ...
    _applyRole(role, actor) {
        ...
        log(`material-panel: apply "${role}" want=${want} onOur=${this._onOurPanel.has(role)}`);
        if (!want) {
        ...
        if (!this._zones && !this._fallbackZone) {
            log(`material-panel: zones not ready for "${role}" — defer`);
            return;
        }
    ...
    _returnToStock(role, actor) {
        try {
            ...
            log(`material-panel: returned "${role}" to stock`);
        } catch (e) {
