    ...
    _reparent(role, actor) {
        ...
        if (placement === 'hidden') {
            try {
                ...
                if (wrap) {
                    // Keep in a zone so show is just visible=true
                ...
                try { actor.visible = false; } catch (e) {}
            } catch (e) {
        ...
        // SHOW — always detach and re-attach so Left/Right/Center moves apply
        try {
            ...
            if (placement === 'right' && typeof zone.addStart === 'function')
                zone.addStart(wrap);
            else if (placement === 'left' && typeof zone.add === 'function')
                zone.add(wrap); // end of left zone = right edge of left pill
            else
                zone.add(wrap);

            log(`material-panel: placed "${role}" in ${placement}`);
        } catch (e) {
