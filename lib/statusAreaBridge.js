        ...
        for (const [role, actor] of toRestore) {
            try {
                ...
                if (typeof add === 'function') {
                    // position 1, box 'right' — same defaults many extensions use
                    add.call(Main.panel, role, actor, 1, 'right');
                    log(`material-panel: restored to stock panel: "${role}"`);
                }
            } catch (e) {
                logError(e, `material-panel: failed to restore "${role}" — toggle that extension or log out`);
            }
        }
