...
export class StatusAreaBridge {
    constructor() {
        ...
        // role -> {box, position} as originally registered
        this._roleOrigin = new Map();
    }
    ...
    disable() {
        // 1) Un-hide GNOME system indicators we suppressed while enabled
        ...
        // 2) Collect every foreign indicator we hold (not only "adopted")
        const toRestore = [];
        for (const [role, actor] of this._mounted.entries()) {
            if (!actor || isBlockedRole(role))
                continue;
            if (isMaterialPanelActor(actor))
                continue;
            toRestore.push([role, actor]);
        }

        // 3) Unbind + unwrap so actors are free (do not destroy them)
        for (const [role, actor] of toRestore) {
            try {
            ...
            // Undo our hide if we had forced visible=false
            try {
        ...
        // 4) Restore stock addToStatusArea BEFORE re-adding
        const add = this._originalAddToStatusArea;
        if (add)
            Main.panel.addToStatusArea = add;
        this._originalAddToStatusArea = null;

        // 5) Put every tray indicator back on the stock panel
        for (const [role, actor] of toRestore) {
            try {
                if (Main.panel.statusArea?.[role] === actor) {
                    try { actor.visible = true; } catch (e) {}
                    log(`material-panel: restore skip (already in statusArea): "${role}"`);
                    continue;
                }
                // Stale pointer from a previous session blocks re-add
                try {
                ...
                if (typeof add === 'function') {
                    ...
                    const box = origin.box || 'right';
                    const position = origin.position ?? 1;
                    add.call(Main.panel, role, actor, position, box);
                    try { actor.visible = true; } catch (e) {}
                    log(`material-panel: restored "${role}" → box=${box}`);
                }
            } catch (e) {
                logError(e, `material-panel: failed to restore "${role}" — enable that extension again`);
            }
        }
