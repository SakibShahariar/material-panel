...
export class StatusAreaBridge {
    ...
    /** Called from vendored AppIndicator when an icon is created. */
    registerSniIcon(role, actor) {
        if (!role || !actor)
            return;
        this._known.set(role, actor);
        try {
            // Ensure Tray page can list it
            this._persistRolesFile();
        } catch (e) {}
        // Apply placement: strip (default right) or explicit zone
        try {
            this._applyRole(role, actor);
        } catch (e) {
            logError(e, 'material-panel: registerSniIcon apply');
        }
    }
    ...
    disable() {
        try { globalThis._materialPanelRegisterSni = null; } catch (e) {}
    ...
    reattachAll() {
        try { this._scanExistingIndicators();
        globalThis._materialPanelRegisterSni = (role, actor) => {
            try { this.registerSniIcon(role, actor); } catch (e) {}
        };
 } catch (e) {}
    ...
    _resolveZone(placement, role = null) {
        // AppIndicator: default/right → SNI strip; left/center → zone (Tray prefs)
        if (role && typeof isBackgroundAppTrayRole === 'function'
            && isBackgroundAppTrayRole(role)
            && this._sniStrip
            && (!placement || placement === 'right')) {
            return this._sniStrip;
        }
