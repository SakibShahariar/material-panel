...
// Stock GNOME roles we already replace (or that only belong on the hidden
// top bar). Never dump these into the Material Panel fallback zone.
...
function isBlockedRole(role) {
    ...
    if (ROLE_BLOCKLIST.has(role))
        return true;
    // Ubuntu/AppIndicator support internal chrome
...
export class StatusAreaBridge {
    ...
    enable() {
        ...
        // Adopt anything already in statusArea (late enable)
        try {
    ...
    _ensureWrapper(role, actor) {
        ...
        try {
            ...
            // Kill stock panel chrome; keep actor reactive for clicks
            if (actor.set_style)
                actor.set_style(
    ...
    _reparent(role, actor) {
        ...
        // Explicit claim wins; else fallback (tray / other extensions)
