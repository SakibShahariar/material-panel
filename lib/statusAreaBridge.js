...
export class StatusAreaBridge {
    ...
    _ensureWrapper(role, actor) {
        ...
        // reactive:false — PanelMenu.Button ClickGesture must receive the click.
        // A reactive wrapper steals events → need multiple clicks / dead menus.
        ...
        try {
            ...
            // Ensure the indicator itself is reactive
            actor.reactive = true;
        ...
        try {
            // Keep menu in the shell menu manager so one click opens reliably
            if (actor.menu && Main.panel.menuManager?.addMenu)
                Main.panel.menuManager.addMenu(actor.menu);
        } catch (e) {}
        try {
