...
export class StatusAreaBridge {
    ...
    _wantedPlacement(role) {
        ...
        if (isBlockedRole(role))
            return null;
        // Extensions' own panel buttons → never host (user wants apps only)
        ...
        // Background apps: auto-adopt into right / tray drawer
        return 'right';
    }
    ...
    _onButtonAdded(role, actor) {
        ...
        this._known.set(role, actor);
        if (!isBackgroundAppTrayRole(role)) {
            log(`material-panel: ignore non-app tray role "${role}" (extension or other)`);
            this._persistRolesFile();
            return;
        }
        log(`material-panel: seen app tray role "${role}"`);
        this._applyRole(role, actor);
        this._persistRolesFile();
    }
