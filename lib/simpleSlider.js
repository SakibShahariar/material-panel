...
function styleCursor(brush, container) {
    const W = brush.width, H = 28, TH = 10, TY = (28 - 10) / 2;
    container.width = W;
    container.height = H;
    css(container, `width: ${W}px; height: ${H}px;`);

    const track = mk(container, W - 8, TH);
    setPos(track, 4, TY);
    css(track, `width: ${W - 8}px; height: ${TH}px; border-radius: 999px; background-color: ${trackBg(0.2)};`);

    const fill = mk(container, 0, TH);
    setPos(fill, 4, TY);

    const capL = mk(container, 14, 14);
    const capR = mk(container, 14, 14);
    const knob = mk(container, 14, 20);

    const paint = () => {
        const a = primary();
        css(fill, `height: ${TH}px; border-radius: 999px; background-color: ${a};` +
            ` box-shadow: 0 0 10px ${rgba(a, 0.45)};`);
        css(knob, `width: 14px; height: 20px; border-radius: 7px; background-color: ${onAccent()};` +
            ` border: 2px solid ${a}; box-shadow: 0 2px 8px ${dropShadow(0.4)};`);
        css(capL, `width: 14px; height: 14px; border-radius: 50%; background-color: ${surfaceSolid()};` +
            ` border: 2px solid ${outlineSoft(0.35)};`);
        css(capR, `width: 14px; height: 14px; border-radius: 50%; background-color: ${surfaceSolid()};` +
            ` border: 2px solid ${outlineSoft(0.35)};`);
    };
    paint();
    setPos(capL, 0, (H - 14) / 2);
    setPos(capR, W - 14, (H - 14) / 2);
    return {
        update(v) {
            v = clamp01(v);
            paint();
            const inner = W - 8;
            const x = Math.round(inner * v);
            fill.width = Math.max(0, x);
            setPos(fill, 4, TY);
            setPos(knob, Math.max(2, Math.min(W - 16, 4 + x - 7)), (H - 20) / 2);
        },
        axis: 'x',
        ratioFromEvent: e => xR(container, W, e),
        scrollStep: 0.02,
...
function styleRails(brush, container) {
    const H = 100, TW = 12, GAP = 10, PAD = 4;
    ...
    const mkRail = (x) => {
        ...
        return {track, f, knob, x};
    };
    const left = mkRail(PAD);
    const right = mkRail(PAD + TW + GAP);

    const paint = () => {
        const a = primary();
        for (const rail of [left, right]) {
            css(rail.f, `width: ${TW}px; border-radius: 8px; background-gradient-direction: vertical;` +
                ` background-gradient-start: ${rgba(a, 0.75)}; background-gradient-end: ${a};`);
            css(rail.knob, `width: 18px; height: 18px; border-radius: 8px; background-color: ${onAccent()};` +
                ` box-shadow: 0 2px 8px ${dropShadow(0.45)}; border: 2px solid ${a};`);
        }
    };
    paint();
    return {
        update(v) {
            ...
            for (const rail of [left, right]) {
                rail.f.height = fh;
                setPos(rail.f, rail.x, H - fh);
                const ky = Math.max(-2, Math.min(H - 16, H - fh - 9));
                setPos(rail.knob, rail.x - 3, ky);
            }
        },
        axis: 'y',
...
function styleWavy(brush, container) {
    ...
    canvas.connect('draw', (_c, cr, w, h) => {
        try {
            ...
            // trough
            cr.set_source_rgba(tr, tg, tb, 0.16);
            cRounded(cr, 2, 8, w - 4, h - 16, 10);
            cr.fill();
            const v = clamp01(value);
            const fx = 6 + (w - 12) * v;
            const base = h - 10;
            // liquid body under wave
            ...
            for (let i = 0; i <= steps; i++) {
                ...
                const wave = Math.sin(x * 0.18 + v * 6) * 4.5;
                cr.lineTo(x, base - 6 - wave - v * 8);
            }
            cr.lineTo(fx, base);
            cr.close_path();
            cr.fill();
            ...
            cr.stroke();
            // knob
            const wave = Math.sin(fx * 0.18 + v * 6) * 4.5;
            const cy = base - 6 - wave - v * 8;
            const [kr, kg, kb] = hexRgb(onAccent());
            cr.set_source_rgba(ar, ag, ab, 0.9);
            cr.arc(fx, cy, 6, 0, TWO_PI);
            cr.fill();
            cr.set_source_rgba(kr, kg, kb, 1);
            cr.arc(fx, cy, 3.5, 0, TWO_PI);
            cr.fill();
        } catch (e) {}
        return true;
    });
...
//  Builder: arc ring dial  (A) — Cairo
// ————————————————————————————————————————————————————————
function styleArc(brush, container) {
    if (!Clutter.Canvas)
        return styleClassic(brush, container);
    const S = 100, CX = S / 2, CY = S / 2, R = 36, TH = 10;
    container.width = S;
    container.height = S;
    ...
    canvas.connect('draw', (_c, cr, w, h) => {
        try {
            ...
            cr.arc(CX, CY, R + TH / 2 + 6, 0, TWO_PI);
            cr.fill();
            // inactive ring
            ...
            const span = clamp01(value) * TWO_PI;
            if (span > 0.001) {
                cr.set_source_rgba(ar, ag, ab, 1);
                cr.set_line_width(TH);
                cr.set_line_cap(Cairo.LineCap.ROUND);
            ...
            cr.set_source_rgba(ar, ag, ab, 1);
            cr.arc(CX + Math.cos(ang) * R, CY + Math.sin(ang) * R, 7, 0, TWO_PI);
            cr.fill();
            cr.set_source_rgba(or_, og, ob, 1);
            cr.arc(CX + Math.cos(ang) * R, CY + Math.sin(ang) * R, 4, 0, TWO_PI);
            cr.fill();
        } catch (e) {}
        return true;
    });
    ...
    css(tx, `font-size: 18px; font-weight: 700; color: ${textColor()};`);
    ...
    return {
        update(v) {
            value = clamp01(v);
            tx.text = pct(v);
            try { css(tx, `font-size: 18px; font-weight: 700; color: ${textColor()};`); } catch (e) {}
            try { canvas.invalidate(); } catch (e) {}
        },
        axis: 'polar',
        ratioFromEvent(e) {
            ...
            let ang = Math.atan2(dy, dx); // -PI..PI, 0 = east
            let t = (ang + Math.PI / 2) / TWO_PI; // 0 at top
            if (t < 0)
                t += 1;
            return t;
        },
