...
export function createSlider({initialValue = 0, onChange, width = DEFAULT_TRACK_WIDTH}) {
    const TRACK_WIDTH = Math.max(40, Math.round(width));
    ...
    try {
        track.style = `width: ${TRACK_WIDTH}px; height: ${TRACK_HEIGHT}px; background-color: rgba(255,255,255,0.12); border-radius: 999px;`;
    } catch (e) {}
    ...
    try {
        fill.style = `height: ${TRACK_HEIGHT}px; background-color: ${globalThis._materialPanelPrimary ?? '#89b4fa'}; border-radius: 999px;`;
    } catch (e) {}
    ...
    try {
        knob.style = `width: ${KNOB_SIZE}px; height: ${KNOB_SIZE}px; background-color: #ffffff; border-radius: 999px; box-shadow: 0 1px 4px rgba(0,0,0,0.45);`;
    } catch (e) {}
    ...
    const updateVisual = v => {
        ...
        knob.set_position(Math.max(0, Math.min(TRACK_WIDTH - KNOB_SIZE, fillPx - KNOB_SIZE / 2)), ky);
    };
    ...
    const tryGrab = (event, actor) => {
        try {
            ...
            logError(e, 'material-panel: slider grab failed');
        }
        return false;
    };
    const tryUngrab = event => {
        try {
            ...
            logError(e, 'material-panel: slider ungrab failed');
        }
        return false;
    };
    const attachStageDrag = () => {
        ...
        stageReleaseId = global.stage.connect('captured-event', (_actor, ev) => {
            ...
            if (ev.type() === Clutter.EventType.BUTTON_RELEASE) {
                dragging = false;
                tryUngrab(ev);
                // Disconnect handlers immediately to avoid re-entrancy issues
                if (stageMotionId) {
