...
function styleArc(brush, container) {
    // All ring paint is on the canvas. Do NOT put a bordered St disc on top —
    // children paint above content and were hiding the primary progress arc.
    const S = 112, CX = S / 2, CY = S / 2, R = 40, TH = 12;
    ...
    // Center fill only (no border) — sits under the ring stroke
    const hole = mk(container, S - TH * 2 - 4, S - TH * 2 - 4);
    const holeSize = S - TH * 2 - 4;
    ...
    if (!(Clutter && Clutter.Canvas)) {
        return {
            update(v) {
                value = clamp01(v);
                tx.text = pct(value);
                css(tx, `font-size: 17px; font-weight: 700; color: ${value > 0.02 ? primary() : textColor()};`);
            },
            axis: 'polar',
            ratioFromEvent(e) {
                try {
                    ...
                    let t = (Math.atan2(ey - (ay + CY), ex - (ax + CX)) + Math.PI / 2) / TWO_PI;
                    if (t < 0) t += 1;
                    return t;
                } catch (err) {
                    return value;
    ...
    canvas.connect('draw', (_c, cr, w, h) => {
        try {
            ...
            // Primary progress from 12 o'clock clockwise
            ...
            if (span > 0.015) {
                ...
                cr.set_source_rgba(ar, ag, ab, 1);
                cr.arc(kx, ky, 7, 0, TWO_PI);
                cr.fill();
                cr.set_source_rgba(or_, og, ob, 1);
                cr.arc(kx, ky, 3.8, 0, TWO_PI);
                cr.fill();
            }
            cr.restore();
        } catch (e) {
    ...
    // Paint canvas ABOVE children so the arc is never covered by the hole/label
    // Clutter paints content first then children — so put arc on a child actor instead.
    const arcLayer = new St.Widget({
        width: S,
        height: S,
        reactive: false,
    });
    ...
    setPos(arcLayer, 0, 0);
    // Label must stay on top of arc layer
    try {
        container.set_child_above_sibling(labHost, arcLayer);
    } catch (e) {}
