...
function styleArc(brush, container) {
    // Muted track + primary progress arc + visible tip knob (St widget, not only Cairo).
    const S = 112, CX = S / 2, CY = S / 2, R = 40, TH = 12, KN = 18;
    try { container.set_size(S, S); } catch (e) {}
    container.width = S;
    container.height = S;
    css(container,
        `width: ${S}px; height: ${S}px; min-width: ${S}px; min-height: ${S}px;` +
        ` max-width: ${S}px; max-height: ${S}px;`);

    let value = 0;

    const holeSize = S - TH * 2 - 8;
    const hole = mk(container, holeSize, holeSize);
    setPos(hole, (S - holeSize) / 2, (S - holeSize) / 2);
    css(hole,
        `width: ${holeSize}px; height: ${holeSize}px; border-radius: ${holeSize / 2}px;` +
        ` background-color: ${surfaceSolid()};`);

    const tx = new St.Label({
        text: '0%',
        x_align: Clutter.ActorAlign.CENTER,
        y_align: Clutter.ActorAlign.CENTER,
    });
    css(tx, `font-size: 17px; font-weight: 700; color: ${textColor()};`);
    const labHost = new St.Bin({
        child: tx,
        x_expand: true,
        y_expand: true,
        x_align: Clutter.ActorAlign.CENTER,
    ...
    try { labHost.set_size(S, S); } catch (e) {}
    container.add_child(labHost);
    setPos(labHost, 0, 0);

    // Physical tip knob — always above canvas so it cannot be hidden
    const knob = mk(container, KN, KN);
    const paintKnob = () => {
        const a = primary();
        css(knob,
            `width: ${KN}px; height: ${KN}px; border-radius: ${KN / 2}px;` +
            ` background-color: ${onAccent()}; border: 3px solid ${a};` +
            ` box-shadow: 0 1px 6px ${dropShadow(0.5)};`);
    };
    paintKnob();
    const placeKnob = v => {
        const span = Math.max(0, Math.min(TWO_PI * 0.97, clamp01(v) * TWO_PI * 0.97));
        const ang = -Math.PI / 2 + span;
        const kx = CX + Math.cos(ang) * R - KN / 2;
        const ky = CY + Math.sin(ang) * R - KN / 2;
        setPos(knob, kx, ky);
    };
    placeKnob(0);

    const polarRatio = e => {
        let ax = 0, ay = 0, ex = 0, ey = 0;
        try { [ax, ay] = container.get_transformed_position(); } catch (err) {}
        try { [ex, ey] = e.get_coords(); } catch (err) { return value; }
        const dx = ex - (ax + CX), dy = ey - (ay + CY);
        if (dx === 0 && dy === 0)
            return value;
        let t = (Math.atan2(dy, dx) + Math.PI / 2) / TWO_PI;
        if (t < 0)
            t += 1;
        return t;
    };

    if (!(Clutter && Clutter.Canvas)) {
        return {
            update(v) {
                value = clamp01(v);
                tx.text = pct(value);
    ...
    canvas.connect('draw', (_c, cr, w, h) => {
        try {
            ...
            // Muted full track
            cr.set_source_rgba(tr, tg, tb, 0.28);
            ...
            // Primary progress
            const span = Math.max(0, Math.min(TWO_PI * 0.97, clamp01(value) * TWO_PI * 0.97));
            if (span > 0.01) {
                cr.set_source_rgba(ar, ag, ab, 1);
                cr.set_line_width(TH);
                cr.set_line_cap(Cairo.LineCap.ROUND);
            ...
            try { logError(e, 'material-panel: arc draw'); } catch (e2) {}
        }
        return true;
    });

    const arcLayer = new St.Widget({width: S, height: S, reactive: false});
    try { arcLayer.set_size(S, S); } catch (e) {}
    css(arcLayer, `width: ${S}px; height: ${S}px;`);
    try {
        arcLayer.content = canvas;
    } catch (e) {
        try { container.content = canvas; } catch (e2) {}
    }
    container.add_child(arcLayer);
    setPos(arcLayer, 0, 0);

    // z-order: hole < arcLayer < labHost < knob
    try { container.set_child_above_sibling(arcLayer, hole); } catch (e) {}
    try { container.set_child_above_sibling(labHost, arcLayer); } catch (e) {}
    try { container.set_child_above_sibling(knob, labHost); } catch (e) {}
...
export function sliderStyleFor(id, which) {
    if (!isValidSliderStyle(id))
        id = 'classic';
    // Keep rails/arc as-selected on every layout (end4 dual uses vertical rows for tall styles).
...
export function createSlider({initialValue = 0, onChange, width = DEFAULT_TRACK_WIDTH, style = 'classic', accent = null} = {}) {
    ...
    // Tall dials: BoxLayout often collapses content-only actors — pin size
    if (style === 'arc') {
        try {
            container.set_size(112, 112);
            container.width = 112;
