...
function styleArc(brush, container) {
    // Default-layout circular dial. Size is forced so BoxLayout cannot collapse
    // a content-only actor into a zero-height strip (which looked like "no arc").
    const S = 112, CX = S / 2, CY = S / 2, R = 40, TH = 12;
    try {
        container.set_size(S, S);
    } catch (e) {}
    container.width = S;
    container.height = S;
    css(container,
    ...
    let value = 0;
    const useCanvas = !!(Clutter && Clutter.Canvas);

    // Visible disc always present (even under canvas) so we never get a blank row
    const disc = mk(container, S, S);
    setPos(disc, 0, 0);
    css(disc,
        `width: ${S}px; height: ${S}px; border-radius: ${S / 2}px;` +
        ` background-color: ${surfaceSolid()}; border: ${TH}px solid ${trackBg(0.28)};`);
    ...
    // Center label without BinLayout fighting content
    ...
    if (useCanvas) {
        ...
        canvas.connect('draw', (_c, cr, w, h) => {
            try {
                ...
                // transparent background — disc widget shows through
                // inactive ring already on disc border; draw active arc only
                const span = Math.max(0, Math.min(TWO_PI * 0.999, clamp01(value) * TWO_PI));
                if (span > 0.01) {
                    cr.set_source_rgba(ar, ag, ab, 1);
                ...
                cr.set_source_rgba(ar, ag, ab, 1);
                cr.arc(kx, ky, 9, 0, TWO_PI);
                cr.fill();
                cr.set_source_rgba(or_, og, ob, 1);
                cr.arc(kx, ky, 5, 0, TWO_PI);
                cr.fill();
                cr.restore();
            } catch (e) {
        ...
        // Prefer content for the arc stroke; disc + label stay as children
        try {
            container.content = canvas;
        } catch (e) {
    ...
    const redraw = () => {
        const a = primary();
        // Update disc border toward primary as value rises (readable without canvas)
        const borderCol = value > 0.02 ? a : trackBg(0.28);
        css(disc,
            `width: ${S}px; height: ${S}px; border-radius: ${S / 2}px;` +
            ` background-color: ${surfaceSolid()}; border: ${TH}px solid ${borderCol};`);
        tx.text = pct(value);
        css(tx, `font-size: 17px; font-weight: 700; color: ${textColor()};`);
        if (canvas) {
            try { canvas.invalidate(); } catch (e) {}
