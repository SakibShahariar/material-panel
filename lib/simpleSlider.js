...
function styleArc(brush, container) {
    // Circular dial: muted full track + primary arc length = value + tip knob.
    // Disc border stays muted so partial values never look "full".
    ...
    // Static muted disc (background only — never primary)
    const disc = mk(container, S, S);
    setPos(disc, 0, 0);
    css(disc,
        `width: ${S}px; height: ${S}px; border-radius: ${S / 2}px;` +
        ` background-color: ${surfaceSolid()}; border: ${TH}px solid ${trackBg(0.22)};`);
    ...
    if (!(Clutter && Clutter.Canvas)) {
        // No canvas: approximate with border opacity only (better than lying full ring)
        return {
            update(v) {
                value = clamp01(v);
                tx.text = pct(value);
                css(tx, `font-size: 17px; font-weight: 700; color: ${textColor()};`);
                const a = primary();
                // partial: thicker primary border is wrong; keep muted + tint center text
                css(disc,
                    `width: ${S}px; height: ${S}px; border-radius: ${S / 2}px;` +
                    ` background-color: ${surfaceSolid()}; border: ${TH}px solid ${trackBg(0.22)};`);
                css(tx, `font-size: 17px; font-weight: 700; color: ${value > 0.02 ? a : textColor()};`);
            },
            axis: 'polar',
            ratioFromEvent(e) {
                try {
    ...
    canvas.connect('draw', (_c, cr, w, h) => {
        try {
            ...
            // Full muted track ring (progress background)
            ...
            // Value arc from 12 o'clock clockwise — length = value
            // Leave a tiny gap at full so 100% still reads as a ring, not a blob
    ...
    try {
        ...
        try { logError(e, 'material-panel: arc content'); } catch (e2) {}
    }

    // Disc border always muted — canvas draws the real track + progress
    css(disc,
        `width: ${S}px; height: ${S}px; border-radius: ${S / 2}px;` +
        ` background-color: ${surfaceSolid()}; border: none;`);
