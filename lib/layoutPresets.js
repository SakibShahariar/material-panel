...
export const END4_ZONES = {
    ...
    // focused | resources(cpu,net)+media | workspaces | clock | … | weather + tray/QS
    left: ['focusedWindow'],
    center: ['cpu', 'networkSpeed', 'media', 'workspaces', 'clock'],
...
export function applyLayoutStyle(config, style) {
    ...
    if (snap?.zones) {
        ...
        // End-4 placement is fixed to the recipe until a future layout editor exists
        if (next === LAYOUT_END4)
