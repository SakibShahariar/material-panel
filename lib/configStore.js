            ...
            try {
                ...
                if (preset?.zones) {
                    ...
                    if (preset?.zones?.center) {
                        ...
                        // strip notifications from center (belongs on right)
                        const ni = c.indexOf('notifications');
                        if (ni >= 0)
                            c.splice(ni, 1);
                    }
                    if (preset?.zones?.right) {
                        const r = preset.zones.right;
                        if (!r.includes('notifications') && !(merged.hiddenModules || []).includes('notifications'))
                            r.unshift('notifications');
                    }
                }
            } catch (e) {}
