...
const DEFAULT_CONFIG = {
    ...
    // Independent top-bar sizing controls. Doesn't affect the quick
    // settings popup contents, which are a separate surface.
    // - iconScale: multiplies the 17px base icon size used by bar-level
    //   modules (activities/battery/volume/network/quicksettings trigger)
    // - pillHeight: px height of each zone pill; padding/font size scale
    //   proportionally from this rather than being separate controls
    // - gap: px margin around each pill (distance from screen edge and
    //   between zones)
    panelSize: {
        iconScale: 1.0,
        pillHeight: 26,
