...
export class ConfigStore {
    ...
    load() {
        ...
        try {
            ...
            // Roll back accidental end4 force from a previous build (user did not opt in)
            if (merged.layoutStyle === 'end4' && merged.activePreset !== 'end4') {
                // Only keep end4 if they explicitly use the end4 preset
                const onlyDefault = !merged.presets || Object.keys(merged.presets).length <= 2;
                if (merged.activePreset === 'default' || merged.activePreset == null)
                    merged.layoutStyle = 'default';
            }
            if (merged.panelSize && merged.panelSize.gapSide == null)
                merged.panelSize.gapSide = merged.layoutStyle === 'end4' ? 14 : 0;
            if (merged.layoutStyle === 'default' && merged.panelSize) {
                // restore classic side inset if we had forced 14
                if (merged.panelSize.gapSide === 14)
    ...
    save(config) {
        ...
        out.panelSize = {scale, gapTop, gapBottom, gapSide};
