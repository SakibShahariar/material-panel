...
const DEFAULT_CONFIG = {
    ...
    // 'default' = original strip zones | 'end4' = opt-in floating BarGroups
    layoutStyle: 'default',
    // Quick Settings slider visual style. See lib/sliderStyleMeta.js for the
    // full list ('classic', 'arc', 'rails', 'meter', 'lightsaber', 'wavy',
    // 'carved', 'neon', 'vu', 'snap', 'cursor').
    sliderStyle: 'classic',
    // QS tile visual style — see lib/tileStyleMeta.js
    qsTileStyle: 'classic',
    panelSize: {
        scale: 1.0,
        gapTop: 5,
        gapBottom: 4,
        gapSide: 0,
        // Space between chips inside a zone (px)
        chipGap: 4,
        popupOpacity: 0.92,
    },
    // '24h' or '12h' (AM/PM)
    clockFormat: '24h',
    // Activity (CPU) chip text: 'cpu' | 'cpu-ram' | 'ram'
    activityChip: 'cpu',
    hiddenModules: [],
    ...
    trayAllHidden: false,
    // Omarchy-style chevron drawer for foreign tray icons
    trayDrawer: true,
    // Tray icons: use system AppIndicator extension (no built-in watcher)
    ...
    presets: {
        ...
        // Opt-in only — never applied unless user sets activePreset or layoutStyle
        // Keep in sync with layoutPresets.END4_ZONES
        end4: {
            zones: {
...
export class ConfigStore {
    ...
    load() {
        ...
        if (!activeFile.query_exists(null)) {
            // Do not write on first read — avoids watcher → full rebuild loop
        ...
        try {
            ...
            const cleanPanelSize = {
                ...
                gapSide: parsed.panelSize?.gapSide ?? DEFAULT_CONFIG.panelSize.gapSide ?? 0,
                chipGap: parsed.panelSize?.chipGap ?? DEFAULT_CONFIG.panelSize.chipGap ?? 4,
                popupOpacity: parsed.panelSize?.popupOpacity ?? DEFAULT_CONFIG.panelSize.popupOpacity ?? 0.92,
            };
            ...
            {
                let cg = Number(cleanPanelSize.chipGap);
                if (!Number.isFinite(cg)) cg = 4;
                cleanPanelSize.chipGap = Math.max(0, Math.min(16, Math.round(cg)));
                let po = Number(cleanPanelSize.popupOpacity);
                if (!Number.isFinite(po)) po = 0.92;
                cleanPanelSize.popupOpacity = Math.max(0.5, Math.min(1, po));
                let gs = Number(cleanPanelSize.gapSide);
                if (!Number.isFinite(gs)) gs = 0;
                cleanPanelSize.gapSide = Math.max(0, Math.min(48, Math.round(gs)));
            }
            ...
            if (merged.trayDrawer == null) merged.trayDrawer = true;
            ...
            if (merged.layoutStyle == null)
                merged.layoutStyle = 'default';
            if (merged.panelSize && merged.panelSize.gapSide == null)
                merged.panelSize.gapSide = 0;

            // Ensure network speed is on the panel if not hidden and missing from zones
            try {
                const preset = merged.presets?.[merged.activePreset];
                if (preset?.zones) {
                    const ids = ['left', 'center', 'right'].flatMap(z => preset.zones[z] || []);
                    if (!ids.includes('networkSpeed') && !(merged.hiddenModules || []).includes('networkSpeed')) {
                        if (!preset.zones.right) preset.zones.right = [];
                        preset.zones.right.unshift('networkSpeed');
                    }
                    if (!ids.includes('btConnected') && !(merged.hiddenModules || []).includes('btConnected')) {
                        if (!preset.zones.left) preset.zones.left = [];
                        if (!preset.zones.left.includes('btConnected'))
                            preset.zones.left.push('btConnected');
                    }
                    // Weather before clock in center
                    if (preset?.zones?.center) {
                        const c = preset.zones.center;
                        const wi = c.indexOf('weather');
                        const ci = c.indexOf('clock');
                        if (wi >= 0 && ci >= 0 && ci < wi) {
                            c[ci] = 'weather';
                            c[wi] = 'clock';
                        }
                        // notifications right of clock in center
                        if (!c.includes('notifications') && !(merged.hiddenModules || []).includes('notifications')) {
                            const clockIdx = c.indexOf('clock');
                            if (clockIdx >= 0)
                                c.splice(clockIdx + 1, 0, 'notifications');
                            else
                                c.push('notifications');
                        }
                    }
                    if (preset?.zones?.right) {
                        const r = preset.zones.right;
                        const ni = r.indexOf('notifications');
                        if (ni >= 0)
                            r.splice(ni, 1);
                    }
                }
    ...
    save(config) {
        ...
        out.panelSize = {scale, gapTop, gapBottom, gapSide, chipGap, popupOpacity};

        out.configVersion = out.configVersion ?? DEFAULT_CONFIG.configVersion ?? 3;
        const file = Gio.File.new_for_path(this._configPath);
        const text = JSON.stringify(out, null, 2);
        // Atomic write: tmp + move so a crash never leaves truncated config.json
        const tmp = Gio.File.new_for_path(this._configPath + '.tmp');
